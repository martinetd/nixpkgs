...
- [Anki Sync Server](https://docs.ankiweb.net/sync-server.html). Added the anki-sync-server service to use the Anki Sync Server built into recent versions of Anki. Enable with [services.anki-sync-server.enable](#opt-services.anki-sync-server.enable).

  This replaces the [`ankisyncd` NixOS module](#opt-services.ankisyncd.enable), but `ankisyncd` is still useful for older Anki clients.
